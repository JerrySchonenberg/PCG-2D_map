...
2. _Generate relief_; first add relief to every row of the map, then smoothen the relief along the rows. The setting `RELIEF_FACTOR` determines how much relief there can be between two pixels.
...
6. _Add plants to the biomes_; trees, bushes, cacti and mushrooms (exclusively to `MYCELLIUM`) are added to the landscape. The probability for vegetation to spawn on a pixel is denoted be `P_PLANT`. Then, based on the biome of the pixel, the type of vegetation is determined. Moreover, for cacti the probability to spawn is lower: there is only `P_CACTI` probability that a cacti gets generated after it is decided whether a plant should be generated.
7. _Add villages to the landscape_; for every pixel is a probability `P_VILLAGE` that is becomes the origin of a village. Then, if this is case, in a radius of `SIZE_VILLAGE_X` and `SIZE_VILLAGE_Y` for every pixel is determined whether it becomes a house (probability is `P_HOUSE`. The origin of the village is saved for later. Villages can only generate in the `GRASS` biome and not on water.
...
The directory `img` contains multiple examples of generated landscapes. All examples are with a resolution of 128x64. The image named `1024.png` is the outlier here, it has a resolution of 1024x1024 and illustrates that the settings must be finetuned in order to get realistic landscapes at different resolutions. 
